...
(function () {
  ...
  /* Kuratierte lizenzfreie Fotos (Unsplash-Lizenz), direkt vom Bild-CDN.
     Im Admin pro Eintrag durch eigene Bilder ersetzbar. */
  const CDN = 'https://images.unsplash.com/photo-';
  function UNS(id, w, h) {
    if (String(id).indexOf('http') === 0) return id;
    const maxw = Math.max(640, Math.round((window.innerWidth || 1200) * 1.4));
    const bw = Math.min(w || 1200, maxw);
    const bh = h ? Math.round(h * (bw / (w || 1200))) : 0;
    return CDN + id + '?w=' + bw + (bh ? '&h=' + bh : '') + '&fit=crop&auto=format&q=62';
  }
  const PIC = {
    hero: '1571080096581-53aefc318ac3',
    fussball: '1606925797300-0b35e9d1794e',
    boxen: '1636581563867-1ecab574858f',
    boxen2: '1716306886418-f84f6d4c2f3a',
    tischtennis: '1770189993553-9537c0846d05',
    gymnastik: '1742249715229-0ce01dd19358',
    gymnastik2: '1747336406564-717968046260',
    team: '1641159009736-8a5fd4e52fef',
    team2: '1529929344488-5e812ddb6e2a',
    teamfoto: '1748111405983-d1b10316eb83',
    kinder: '1622659097509-4d56de14539e',
    damen: '1535506197904-e5c09c0e5619',
    spiel: '1600077063877-22118d6290eb',
    platz: '1478851814281-61a21b884e62'
  };
  ...
  const crest = (cls) => C.settings.logoUrl
    ? `<img class="crest-img ${cls || ''}" src="${esc(C.settings.logoUrl)}" alt="Wappen ${esc(C.settings.clubName)}" onerror="this.outerHTML='<span class=\\'crest\\' aria-hidden=\\'true\\'><b>ÖSG</b><small>08</small></span>'" />`
    : `<span class="crest ${cls || ''}" aria-hidden="true"><b>ÖSG</b><small>08</small></span>`;
  ...
  function home() {
    ...
    <section class="section">
      <div class="wrap wrap--wide">
        <div class="section-head">
          <div><span class="kicker">Spieltag & Kalender</span><h2 class="section-title">Vom Platz<br />und drumherum</h2></div>
          <a class="eyebrow-link" href="#/termine">Alle Termine <span class="arr">→</span></a>
        </div>
        <div class="panel">
    ...
    <section class="section alt">
      <div class="wrap wrap--wide">
        <div class="section-head">
          <div><span class="kicker">Aktuelles</span><h2 class="section-title">News aus<br />dem Verein</h2></div>
          <a class="eyebrow-link" href="#/news">Alle News <span class="arr">→</span></a>
        </div>
        <div class="news-grid three">${sortedNews().slice(0, 3).map(newsCard).join('')}</div>
      </div>
    </section>
  ...
  function kontakt(query) {
    ...
    const teamGroups = [...new Set(fbTeams.map((t) => t.group || 'Mannschaften'))].map((g) =>
      `<optgroup label="${esc(g)}">${fbTeams.filter((t) => (t.group || 'Mannschaften') === g)
        .map((t) => `<option>${esc(t.name)}${t.league ? ' (' + esc(t.league) + ')' : ''}</option>`).join('')}</optgroup>`).join('');
    ...
    <section class="section"><div class="wrap form-grid">
      <form class="form" id="anfrageForm" novalidate>
        ...
        <div class="f2">
          ...
          <label>Abteilung<select name="dept" id="deptSel"><option value="">bitte wählen</option>${deptOpts}</select></label>
        </div>
        <label id="teamWrap"${pre === 'Fußball' ? '' : ' hidden'}>Mannschaft (optional)
          <select name="team"><option value="">weiß ich noch nicht</option>${teamGroups}</select>
        </label>
        <div class="f2">
  ...
  function wireForm() {
    ...
    if (!f) return;
    const dept = document.getElementById('deptSel');
    ...
    f.addEventListener('submit', (e) => {
      ...
      const payload = { type: d.type, dept: d.dept, team: d.dept === 'Fußball' ? (d.team || '') : '', name: d.name, birth: d.birth, email: d.email, phone: d.phone, message: d.message };
